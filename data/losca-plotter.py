...
ax.plot(tl[1:], g1l[1:], label=r'Beginning of life')
...
#ax.set_xscale('log')
#ax.set_yscale('log')
ax.plot(t[1:], temp[1:], label=r'Start-up')
ax.plot(tl[1:], templ[1:], label=r'Beginning of life')
ax.plot(tq[1:], tempq[1:], label=r'Equilibrium')
#ax.plot(te[1:], maxe[1:])
#ax.plot(ts[1:], temps[1:], label=r'Step')
ax.set_xlabel(r'Time [s]')
ax.set_ylabel(r'Average fuel temperature [K]')
ax.legend()
plt.savefig('loscatemp.png', dpi=400)

fig, ax = plt.subplots()
ax.plot(t[1:], heat[1:], label=r'Start-up')
ax.plot(tl[1:], heatl[1:], label=r'Beginning of life')
ax.plot(tq[1:], heatq[1:], label=r'Equilibrium')
#ax.plot(ts[1:], heats[1:], label=r'Step')
ax.set_xlabel(r'Time [s]')
ax.set_ylabel(r'Total thermal power [W]')
ax.legend()
#ax.set_xscale('log')
#ax.set_yscale('log')
